...
# Separate eval env (deterministic, no noise)
...
agent     = TD3(config, trans_config, buffer, device)
...
for step in range(config.training_iterations):
    ...
    # 1. Select action
    actions = agent.select_action(observations, explore=True)   # numpy (n_envs, act_dim)

    # 2. Step environments
    next_obs_np, rewards, terminations, truncations, infos = envs.step(actions)
    dones = terminations | truncations                          # (n_envs,) bool array

    # 3. Store transitions (handle gym auto-reset: use real next_obs before reset)
    # gymnasium AsyncVectorEnv puts the true terminal obs in infos["final_observation"]
    ...
    # Reset episode rewards for finished envs
    ...
    # 6. Periodic deterministic evaluation (every 5000 steps)
